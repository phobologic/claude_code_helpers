...
**Team lead CWD is sacred.** Spawned agents inherit the team lead's CWD as their
starting directory, and Bash CWD persistence only works when the agent starts from
the repo root. If the team lead's CWD drifts (e.g. by `cd`-ing into a worktree for
merges), all subsequently spawned agents will have broken CWD tracking. Therefore:
- **Never `cd` into a worktree from the team lead.** Use `git -C <path>` for merge
  operations instead.
- Before spawning agents at wave boundaries, verify with `cd $REPO_ROOT && pwd`.
...
lead operates from the main repo and must use `git -C <path>` when acting on a
worktree — never `cd <path> && git`. Never use `git -C` in implementer prompts.
