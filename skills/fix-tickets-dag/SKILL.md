...
3. Implement the fix and run tests from your worktree in this same turn.
...
#    `--detach` is essential: a regular checkout would pin
#    fix/batch-<stamp> to this worktree, and the next CLEAN handler's
#    main-repo merge step (`git -C $REPO_ROOT checkout fix/batch-<stamp>`)
#    would fail with "fatal: 'fix/batch-<stamp>' is already used by
#    worktree at ...".
git -C <worktree> checkout --detach fix/batch-<stamp>
...
> merge step does `git -C $REPO_ROOT checkout fix/batch-<stamp>` in the
> main repo. If a freed implementer worktree were left on the integration
> branch, the next merge would fail. Detached HEAD leaves the working tree
> on the right commit without holding the branch ref. When a new ticket is
> later dispatched to this slot, the dispatch script runs `git checkout
> fix/<id>` (or creates it from `fix/batch-<stamp>`), which works the same
